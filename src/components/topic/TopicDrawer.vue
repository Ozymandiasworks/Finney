      ...
      <q-list>
        ...
        <q-item>
          <q-item-section>
            ...
            <q-item-label caption>({{ transformedFilter }} XEC)</q-item-label>
          </q-item-section>
          <q-item-section>
            <q-slider
              v-model="filterModel"
        ...
        <q-item>
          <q-item-section>
            ...
            <q-item-label caption>({{ transformedOffering }} XEC)</q-item-label>
          </q-item-section>
          <q-item-section>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const updateModels = () => {
      const threshold = topics.value[props.topic]?.threshold / 100
      filterModel.value =
        threshold === 0
          ? 0
          : Math.sign(threshold) * Math.log10(Math.abs(threshold))

      const offering = topics.value[props.topic]?.offering / 100
      offeringModel.value =
        offering === 0
          ? 1
    ...
    watch(transformedFilter, newValue => {
      topicStore.topics[props.topic].threshold = newValue * 100
    })

    watch(transformedOffering, newValue => {
      topicStore.topics[props.topic].offering = newValue * 100
    })
