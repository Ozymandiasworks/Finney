...
1. Continue dependency remediation in small compatibility groups. Electron 44.3.0, Node 24.19.0, direct `node-forge` 1.4.0 and the transitive `elliptic` 6.6.1 resolution are now merged after clean frozen installs, core-suite runs and full Windows builds. Next, classify the `cipher-base`, `pbkdf2` and `sha.js` chain independently because it spans wallet mnemonic and browser-polyfill paths. Dependabot PR #3 (`ea2f868`) remains unsuitable for merge because it also upgrades Quasar and related tooling; its bypassed-engine build failed in the upgraded Quasar loader and Sass pipeline.
