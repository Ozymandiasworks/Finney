...
Status: design only. No production encryption library, message format, or wallet code changes are included here.
...
The encrypted record format, master-key handling and key rotation must be chosen together with the local-privacy work. The storage interface should expose `get`, `getMany`, `put`, `del` and one atomic `batch` operation; callers must not get direct plaintext LevelDB access.
