...
2. Build the main-process integration spike for the selected AGPL `@signalapp/libsignal-client` direction, including packaged Electron native-module validation. The detailed protocol, identity, stamp and storage boundary is in `MODERN_ENCRYPTION_MIGRATION.md`.
3. Implement protected private-state storage and the staged Signal store adapter before extending the profile, relay and message schemas. The durable-session and paid-outbox invariants in `DURABLE_SESSION_PERSISTENCE.md` remain mandatory.
