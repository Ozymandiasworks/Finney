...
import { PrivateKey } from 'bitcore-lib-xec'
...
test('Stamp key recovery', () => {
  const destinationPrivateKey = PrivateKey()
  const destinationPublicKey = destinationPrivateKey.toPublicKey()
  const payloadDigest = Buffer.from(
    'b8c8e1d1bb8c5fb7826c362c19f8ef72f7f54ac659c609c48998d83c95e27771',
    'hex',
  )

  const senderSide = payloadConstructor.constructStampPublicKey(
    payloadDigest,
    destinationPublicKey,
  )
  const receiverSide = payloadConstructor.constructStampPrivateKey(
    payloadDigest,
    destinationPrivateKey,
  )

  expect(senderSide.toBuffer()).toStrictEqual(
    receiverSide.toPublicKey().toBuffer(),
  )
})
