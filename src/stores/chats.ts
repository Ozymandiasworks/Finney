...
export const useChatStore = defineStore('chats', {
  ...
  actions: {
    ...
    sendMessageLocal({
      ...
      // A contact can exist before its Pinia chat state has ever been created.
      // The legacy code treated every missing chat as a self-send and returned,
      // which made successful outbound messages invisible on the sender side.
      let chat = this.chats[displayAddress]
      if (!chat) {
        const senderDisplayAddress = toDisplayAddress(senderAddress)
        if (senderDisplayAddress === displayAddress) {
          // Preserve the inherited self-send behavior.
          return
        }
        chat = {
          ...defaultContactObject,
        ...
        this.chats[displayAddress] = chat
      }
