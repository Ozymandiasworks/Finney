...
const dustLimit = 546 // Standard eCash P2PKH dust threshold (5.46 XEC)
...
export class Wallet {
  ...
  chronikPollingTimer: ReturnType<typeof setInterval> | undefined
  chronikPollingInFlight = false
  ...
  async init() {
    ...
    try {
      ...
      console.warn(
        'Chronik WebSocket unavailable; using HTTP UTXO polling fallback',
        err,
      )
      this.startChronikPollingFallback()
    }
  ...
  async updateUTXOsFromTxid(txid: string) {
    ...
    try {
      ...
      for (const [outIdx, txOutput] of tx.outputs.entries()) {
        const script = Script.fromHex(txOutput.outputScript)
      ...
      for (const txInput of tx.inputs) {
        if (!txInput.outputScript) {
          continue
        }
        const script = Script.fromHex(txInput.outputScript)
        if (!script.isPublicKeyHashOut()) {
          continue
        }
        const pkh = script.getPublicKeyHash().toString('hex')
  ...
  async updateUTXOsFromPkh(pkh: string) {
    ...
    try {
      ...
      const utxos: Utxo[] = chronikUtxos.utxos.map(utxo => ({
  ...
  startChronikPollingFallback(intervalMs = 15000) {
    if (this.chronikPollingTimer !== undefined) {
      return
    }

    const refresh = async () => {
      if (this.chronikPollingInFlight) {
        return
      }
      this.chronikPollingInFlight = true
      try {
        await this.updateHDUTXOs()
      } catch (err) {
        console.error('Chronik HTTP polling refresh failed', err)
      } finally {
        this.chronikPollingInFlight = false
      }
    }

    console.warn(`Chronik HTTP polling fallback active every ${intervalMs} ms`)
    void refresh()
    this.chronikPollingTimer = setInterval(() => {
      void refresh()
    }, intervalMs)
  }
  ...
  async startListeners() {
    ...
    // Current Chronik requires the websocket to be open before we register
    // script subscriptions. The endpoint automatically reconnects and restores
    // subscriptions after this initial connection.
    await chronikWs.waitForOpen()

    for (const key of [...this.walletKeys, ...this.changeKeys]) {
      chronikWs.subscribeToScript(
        'p2pkh',
  ...
  async checkAndFixUtxos(utxos: Utxo[], unfreeze = false): Promise<boolean[]> {
    ...
    // validateUtxos was removed from modern Chronik. Revalidate each stored
    // outpoint against the current UTXO set for its eCash address instead.
    // A failed Chronik request throws before anything is deleted, so a network
    // outage cannot make Finney discard wallet state.
    const currentByAddress = new Map<string, Set<string>>()
    for (const address of [...new Set(utxos.map(utxo => utxo.address))]) {
      const response = await chronikClient.address(address).utxos()
      currentByAddress.set(
        address,
        new Set(
          response.utxos.map(
            utxo => `${utxo.outpoint.txid}_${utxo.outpoint.outIdx}`,
          ),
        ),
      )
    }
    ...
    return utxos.map((utxo, idx) => {
      ...
      const isUnspent = currentByAddress.get(utxo.address)?.has(utxoId) === true
      if (isUnspent) {
      ...
      console.log('UTXO no longer unspent, deleting local UTXO', utxoId)
  ...
  async addressUpdated(msg: WsMsgClient) {
    switch (msg.type) {
      case 'Tx':
        switch (msg.msgType) {
          ...
          case 'TX_INVALIDATED':
            // Refresh all watched scripts because the tx may no longer be a
            // valid source of wallet UTXOs.
            await this.updateHDUTXOs()
            return
          default:
            return
      ...
      case 'Error':
        console.error('Error from ws:', msg.msg)
        return
      case 'Block':
        return
    }
  ...
  async forwardUTXOsToPubkey({
    utxos,
    pubkey,
    allowUnstoredUtxos = false,
  }: {
    utxos: Utxo[]
    pubkey: PublicKey
    allowUnstoredUtxos?: boolean
  }) {
    ...
    for (const passedInUtxo of utxos) {
      const storedOutpoint: BuildableUtxo | undefined = this.storage.getById(
        calcUtxoId(passedInUtxo),
      )
      const outpoint: BuildableUtxo | undefined = storedOutpoint
        ? storedOutpoint
        : allowUnstoredUtxos
        ? ({ ...passedInUtxo } as BuildableUtxo)
        : undefined
      if (!outpoint) {
        continue
      }
  ...
  get displayAddress() {
    // TODO: This should be in the relay client, not the wallet...
    // TODO: Not just testnet
    return this.identityPrivKey?.toAddress(this.networkName).toCashAddress()
  }
