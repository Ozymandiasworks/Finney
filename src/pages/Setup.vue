...
import { HDPrivateKey } from 'bitcore-lib-xec'
...
export default defineComponent({
  ...
  methods: {
      ...
      try {
        ...
        this.relayData = {
          ...
          profile: {
            ...
            name: this.accountData.name || 'Finney User',
            avatar: this.avatar,
          },
        }
        if (!err.response) {
    ...
    async setupRelay() {
      ...
      if (isLocalDevelopmentUrl(this.relayUrl)) {
        // The localhost relay is an in-memory development service. Do not spend
        // XEC merely to authorize a local protocol test. Message stamps remain
        // part of actual messages and are tested separately.
        relayClient.setToken(LOCAL_DEVELOPMENT_TOKEN)
        this.setRelayToken(LOCAL_DEVELOPMENT_TOKEN)
        this.$relayClient.setToken(LOCAL_DEVELOPMENT_TOKEN)
      } else {
  ...
  computed: {
    forwardEnabled() {
      // The EULA is entirely local. Do not make the Agree button depend on
      // Chronik/network availability. Wallet-dependent setup steps still fail
      // closed until the indexer connection is available.
      if (this.step === 1) {
        return true
      }

      // Account creation/import is local cryptographic work. Do not block the
      // public-name/seed step on Chronik availability. Network readiness is
      // required only once setup reaches network-dependent work.
      if (this.step >= 3 && !this.$indexer.connected) {
        return false
      }
