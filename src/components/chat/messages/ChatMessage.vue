    ...
    <template v-if="payloadDigest">
      ...
      >
        ...
        <template #stamp>
          ...
          />
          <q-btn
            v-if="showRedeemStampTest"
            dense
            flat
            size="sm"
            icon="science"
            label="Redeem Stamp Test"
            @click="redeemStampTest"
          />
        </template>
      </q-chat-message>
    </template>
...
// The redemption proof is retained for controlled development diagnostics but
// is deliberately hidden from the normal chat UI after the core proof passed.
const showDevelopmentStampControls = false
...
export default defineComponent({
  ...
  methods: {
    ...
    async redeemStampTest() {
      try {
        const result = await this.$relayClient.redeemStampForProof(
          this.payloadDigest,
        )
        this.$q.notify({
          type: 'positive',
          message: `Stamp redemption proof broadcast: ${result.txid}`,
          timeout: 10000,
        })
      } catch (err) {
        console.error('Stamp redemption proof failed', err)
        this.$q.notify({
          type: 'negative',
  ...
  computed: {
    showRedeemStampTest() {
      return (
        showDevelopmentStampControls &&
        !this.message.outbound &&
        this.payloadDigest.length > 0 &&
        this.message.outpoints.some(outpoint => outpoint.type === 'stamp')
      )
    },
    items() {
      const sorted: {
    ...
    stampAmount() {
      ...
      return Number(amount / 100).toFixed(2) + ' XEC'
    },
    bgColor() {
