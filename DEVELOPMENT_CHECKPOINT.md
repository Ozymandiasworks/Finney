...
- An inactive main-owned Signal provisioning routine now creates one separate Signal identity, one-time pre-key, signed pre-key, Kyber pre-key, and public bundle for each identity id. It persists private material through the protected-state store and commits the corresponding public bundle with the staged key records. Repeated provisioning and protected-store restart return the same bundle. It has no Electron startup caller, profile publication, renderer IPC, relay use, or production message path.
- scripts/test-signal-profile-provisioning.mjs verifies stable protected provisioning, restart persistence, and libsignal bundle reconstruction. The full core suite, Windows package build, NSIS installer, and packaged smoke test passed after this addition.
- Gate 1 review snapshot preparation is complete. See GATE1_REVIEW.md. No work may proceed into a new architectural phase until an independent clean-room review closes the gate.
...
- An inactive Electron main-process private-state foundation encrypts each LevelDB value through `safeStorage`, refuses unavailable OS protection and rejects Linux plaintext or unknown keyring backends. It supports encrypted `get`, `getMany`, `put`, `del` and atomic `batch` operations, but has no renderer IPC or production protocol caller. Its Node regression and packaged Windows smoke test passed.
- The inactive staged Signal store adapter serializes identity, session, one-time prekey, signed prekey and Kyber prekey state through the protected store. Library mutations remain in memory until one explicit commit can include future outbox or receipt records. Its libsignal regression covers first-contact provisioning, discarded mutations, persisted session state and prekey consumption. It has no renderer IPC, profile, relay or message-schema caller.
...
2. Define the narrow preload IPC contract for profile-bundle publication, session setup, encrypt and decrypt. Do not expose store objects, serialized keys or a generic cryptographic interface. The durable-session and paid-outbox invariants in `DURABLE_SESSION_PERSISTENCE.md` remain mandatory.
