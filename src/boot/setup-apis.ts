...
} from 'src/utils/local-development'
...
function createAndBindNewIndexerClient({
  ...
  try {
    ...
    void chronikClient
      .blockchainInfo()
      .then(() => console.log('Chronik HTTP connected'))
      .catch(err => console.error('Chronik HTTP unavailable:', err))
...
export default boot(async ({ app }) => {
  ...
  let token = relayStore.token
  // The localhost token is a development authorization shortcut, not proof
  // that wallet/profile setup has completed. Only inject it automatically
  // for an already-configured identity. The setup wizard installs the same
  // token itself after generating a new wallet identity.
  if (status.setup && isLocalDevelopmentUrl(defaultRelayUrl)) {
    token = LOCAL_DEVELOPMENT_TOKEN
    relayStore.setToken(token)
  }
  if (status.setup && token) {
